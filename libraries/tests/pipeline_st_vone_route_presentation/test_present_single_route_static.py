...
def _run_single_route_presentation_test(problem_type_mapping: dict):
    # --------------------------------------------------
    ...
    # --------------------------------------------------
    # Step 4: static presentation (UNDER TEST)
    # --------------------------------------------------
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert len(route_pages) == 1

        # --------------------------------------------------
        # PRINTS (intentional)
        # --------------------------------------------------
        print("\n[TEST] pipeline_st_vone_route_presentation")
        print("-" * 70)
        print(f"Problem type mapping : {problem_type_mapping}")
        print(f"Resolved problem type: {route_prep.problem_type}")
        print(f"Route ID             : {route_id}")
        print(f"Map file             : {map_files[0]}")
        print(f"Route page           : {route_pages[0]}")
        print("-" * 70)
...
# Actual tests
# ---------------------------------------------------------------------
def test_route_presentation_hpp():
    _run_single_route_presentation_test(PROBLEM_TYPE_VARIANTS["HPP"])


def test_route_presentation_out_hpp():
    _run_single_route_presentation_test(PROBLEM_TYPE_VARIANTS["OUT:HPP"])


def test_route_presentation_tsp():
    _run_single_route_presentation_test(PROBLEM_TYPE_VARIANTS["TSP"])


def test_route_presentation_out_tsp():
    _run_single_route_presentation_test(PROBLEM_TYPE_VARIANTS["OUT:TSP"])


if __name__ == "__main__":
    test_route_presentation_hpp()
    test_route_presentation_out_hpp()
    test_route_presentation_tsp()
    test_route_presentation_out_tsp()
